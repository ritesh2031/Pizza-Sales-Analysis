...
select hour(orders.order_time) as hours, count(order_id)
from orders
group by hours;
...
select round(avg(order_quantity),0) as avg_pizzas_ordered_per_day
from (select orders.order_date as date, sum(order_details.quantity) as order_quantity
from order_details
join orders
